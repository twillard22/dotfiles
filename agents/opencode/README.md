...
cd ~/Development/mavely/react-native/mavely-native && opencode
```

In the session: `/models` should show only the configured models; ask "what skills
are available?" and expect `claude-artifact`, `linear-create-project`, `zsh`,
`later-slides`; run a trivial task with `ultrawork` and watch which agents and models
fire.
