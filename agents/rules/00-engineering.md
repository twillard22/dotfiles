...
Linked by `setup.sh` as `~/.claude/rules/00-engineering.md`. Claude Code reads that
directory natively; OpenCode loads it via `instructions` in
`agents/opencode/opencode.jsonc`. Project `AGENTS.md` files add to this. Setup and
how-to docs live in `README.md` and `agents/opencode/README.md`.

> **"Make a note" means persist it in the owning layer.** Use this file for generic
> rules, `~/.work-agents/agents/rules/50-later.md` for work rules,
> `<layer>/agents/repos/<name>/rules.md` for private per-repo notes, and
> `<repo>/AGENTS.md` for team-visible rules. Show the edit.
